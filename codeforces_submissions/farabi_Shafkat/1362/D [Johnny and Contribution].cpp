...
ull ceil_(ull x, ull y)
{

    return x / y + (x % y != 0);
}
vector<ll>graph[6*TEN5 +10];
pll topic[6*TEN5 +10];
ll tpc[6*TEN5 +10];
...
int main()
{
   // FAST;
    ...
    {   ll node=topic[i].ss;
        ...
        {
            if(tpc[next]==tpc[node]){y=0;break;}
            else if(tpc[next]<tpc[node])mp[tpc[next]]++;
        }
        if(tpc[node]-1!=mp.size()){y=0;break;}
        vis_list.emb(node);
    }
    if(!y)
    {
